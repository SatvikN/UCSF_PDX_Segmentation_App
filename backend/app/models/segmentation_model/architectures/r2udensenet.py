...
from tensorflow.keras.layers import Conv2D, Conv2DTranspose, MaxPooling2D, concatenate, Input, Dropout, Add, Activation
...
# Image dimensions for inference
IMAGE_ROW = 192
IMAGE_COL = 192
IMAGE_DEPTH = 1

def rec_layer(layer, filters):
    """Recurrent layer for R2U-DenseNet architecture"""
    ...
    drop_inter = Dropout(0.3)(reconv1)
...
def create_r2udensenet_model():
    """Create R2U-DenseNet model for inference only"""
...
def predict_mask(model, image):
    ...
    if image.shape[1:3] != (IMAGE_ROW, IMAGE_COL):
        image = tf.image.resize(image, [IMAGE_ROW, IMAGE_COL])
    ...
    # Return binary mask (threshold at 0.5)
